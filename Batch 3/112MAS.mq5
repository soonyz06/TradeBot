...
input double TpATR1 = 6; //TP
double TpATR2 = TpATR1; 
input double SlATR = 1; //SL
int TimeExit1 = -1; //Time Exit ///eh
...
void OnInit(){
   ...
   MAHandler1 = iMA(_Symbol, Timeframe, 50, 0, MODE_LWMA, PRICE_CLOSE);
   MAHandler2 = iMA(_Symbol, Timeframe, 50, 0, MODE_SMA, PRICE_CLOSE);
   MAHandler3 = iMA(_Symbol, Timeframe, 400, 0, MODE_SMMA, PRICE_CLOSE); 
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start && TimeCurrent()<=Stop){
         ...
         if(MA1[1]>MA2[1] && MA2[1]>MA3[1] && MA1[2]<MA2[2]){
            if(buyPosition==0)
               ExecuteBuy();
         }
         
          //Exit
         if(MA1[1]<MA2[1] && buyPosition>0){
