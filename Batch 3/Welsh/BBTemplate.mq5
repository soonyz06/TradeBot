      ...
      for(int i=0; i<OrdersTotal(); i++){
         ...
         if(OrderSelect(_ticket)){
            if(PositionGetInteger(POSITION_MAGIC) == Magic && PositionGetString(POSITION_SYMBOL) == _Symbol){
               if(PositionGetInteger(POSITION_TYPE) == ORDER_TYPE_BUY_STOP || PositionGetInteger(POSITION_TYPE) == ORDER_TYPE_BUY_LIMIT){
                  _buyPosition+=1;
               }
               else if(PositionGetInteger(POSITION_TYPE) == ORDER_TYPE_SELL_STOP ||PositionGetInteger(POSITION_TYPE) == ORDER_TYPE_SELL_LIMIT){
                  _sellPosition+=1;
               }
            }
         }
