...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         //CopyBuffer(BBHandler2, 2, 0, 3, Lower);
         //CopyBuffer(BBHandler, 0, 0, 3, Middle);
         CheckPositions();
         Range(lookback);
         
         if(Price[2].close<Upper[2] && Price[1].close>Upper[1] && Price[1].close>Price[1].open && Price[0].open>Price[1].low-ATR[1]){
            if(buyPosition==0)
               ExecuteBuy();
         }
