...
input int TimeExit1 = 30; //Time Exit
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         //Entry
         if(Price[2].close>Upper[2] && Price[1].close<Upper[1] && Price[1].close>MA[1] && Price[2].close>Highest){
            if(sellPosition==0 && buyPosition==0)
               ExecuteSell();
         }
         if(Price[2].close<Lower[2] && Price[1].close>Lower[1] && Price[1].close<MA[1] && Price[2].close<Lowest){
