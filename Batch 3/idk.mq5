...
input int TpPoints1 = 750; //TP
int TpPoints2 = TpPoints1; //TP 2
input int SlPoints = 650; //SL
input int TimeExit1 = 4; //Time Exit
int TimeExit2= TimeExit1;
//10, 4, 2, 3
int TriggerPoints = Mode;
int Confirmation = 0;
...
void OnInit(){
   ...
   MAHandler = iMA(_Symbol, PERIOD_D1, 50, 0, MODE_SMA, PRICE_CLOSE);
...
void OnTick(){
   ...
   //newbar2 = iBars(_Symbol, PERIOD_H1);
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         if(buyPosition+sellPosition==0){
            if(Price[1].close>Price[2].high && Price[0].close>MA[0] && Price[1].close>Price[1].open){
              ExecuteSell();
            }
            if(Price[1].close<Price[2].low && Price[0].close<MA[0] && Price[1].close<Price[1].open){
               ExecuteBuy();
            }
         }
         bar1=newbar;
...
void ExecuteBuy(double slpoints=NULL, double tppoints=NULL){
   ...
   else tppoints = Ask+tppoints*_Point;
...
void ExecuteSell(double slpoints=NULL, double tppoints=NULL){
   ...
   if(tppoints==NULL && Mode==0 && TpPoints1>0) tppoints = Bid-TpPoints1*_Point;
   else tppoints = Bid-tppoints*_Point;
...
void Range(int bars){
   ...
   CopyHigh(_Symbol, PERIOD_D1, 2, bars, High);
   CopyLow(_Symbol, PERIOD_D1, 2, bars, Low);
