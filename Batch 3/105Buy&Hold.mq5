...
void OnInit(){
   ...
   MAHandler_Fast = iMA(_Symbol, Timeframe, len1, 0, MODE_SMA, PRICE_CLOSE);
   MAHandler_Trend = iMA(_Symbol, Timeframe, len2, 0, MODE_SMA, PRICE_CLOSE);
   Comment("ID: ", Magic);
   CheckPositions();
}
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         CopyBuffer(MAHandler_Fast, 0, 0, 3, MA_Fast);
         CopyBuffer(MAHandler_Trend, 0, 0, 3, MA_Trend);
         CheckPositions();
         ...
         if(Strategy==0){
            ...
            if(MA_Fast[1]<MA_Trend[1] && MA_Fast[2]>MA_Trend[2]){
               ExecuteBuy();
            }
         }
