...
input int TpPoints1 = 2000; //TP
int TpPoints2 = TpPoints1; 
input int SlPoints = 1600; //SL
input int TimeExit1 = 20; //Time Exit
int TimeExit2= TimeExit1;
int TriggerPoints;
//500,1500 || 1000, 1500 || 1500, 2000 *MODE 1
//15, 25, 35
...
void OnInit(){
   ...
   MAHandler = iMA(_Symbol, Timeframe, 100, 0, MODE_SMA, PRICE_CLOSE);
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         if(Price[1].close>Price[2].high && buyPosition>0){
            CloseOrder(ORDER_TYPE_BUY_STOP);
         }
         if(Price[1].close<Price[2].low && sellPosition>0){
            CloseOrder(ORDER_TYPE_SELL_STOP);
         }
         
         if(Price[1].close<Price[2].low && Price[1].close>MA[1] && Price[2].close<Price[2].open){
            if(buyPosition+sellPosition==0)
               ExecuteBuyStop(Price[1].high);
         }
         if(Price[1].close>Price[2].high && Price[1].close<MA[1] && Price[2].close>Price[2].open){
            if(buyPosition+sellPosition==0)
