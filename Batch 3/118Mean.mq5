...
double TriggerATR = 0;
input int lookback = 0; //Lookback
//COST AXP XOM

input group "=== Bands 1 ==="
input int bb_len = 40; //Length
input double std = 2; //Standard Deviation

input group "=== Bands 2 ==="
input int bb_len2 = 70; //Length
input double std2 = 1; //Standard Deviation
...
void OnInit(){
   ...
   BBHandler2 = iBands(_Symbol, Timeframe, bb_len2, 0, std2, PRICE_CLOSE);
   Comment("ID: ", Magic);
   CheckPositions();
}
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         CopyBuffer(BBHandler2, 1, 0, 3, Upper);
         //CopyBuffer(BBHandler, 0, 0, 3, Middle);
         CheckPositions();
         Range(lookback);
         
         if(Price[2].close<Lower[2] && Price[1].close>Lower[1] && Price[1].close<Upper[1] && Price[2].close<Lowest){
            if(buyPosition==0)
            ExecuteBuy();
         }
         
         if(Price[2].close>Upper[2] && Price[1].close<Upper[1] && Price[1].close>Lower[1]){
            if(buyPosition>0)
