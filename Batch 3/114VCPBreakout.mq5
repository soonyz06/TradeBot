...
double TriggerATR = 0;
int len = 30; //Length
double std = 2; //Standard Deviation
input int Range = 20;
int Confirmation = Range;
...
void OnInit(){
   ...
   ATRHandler = iATR(_Symbol, PERIOD_D1, 60);///Google+Broad 10, else 50/100 
   BBHandler = iBands(_Symbol, PERIOD_D1, len, 0, std, PRICE_CLOSE); 
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         CheckPositions();
         Range(Range, Confirmation);
         
         if(Void<ATR[1]*3){
         //if(Void<Upper[1]-Lower[1]){
            if(buyPosition==0 && Price[1].close>Highest)
               ExecuteBuy();
         }
...
void Range(int bars, int bars2){
   if(bars<=0 && bars2<=0) return; 
   ...
   Void = Highest-Lowest;
   Highest = iHigh(_Symbol, Timeframe, iHighest(_Symbol, Timeframe, MODE_HIGH, bars2, 2));
   Lowest = iLow(_Symbol, Timeframe, iLowest(_Symbol, Timeframe, MODE_LOW, bars2, 2));
