...
double TriggerATR = 0;
input int len = 10; //Length
input bool Confirmation = false;
...
void OnInit(){
   ...
   ATRHandler = iATR(_Symbol, PERIOD_MN1, 10);
   MAHandler = iMA(_Symbol, Timeframe, len, 0, MODE_SMA, PRICE_CLOSE);
   Comment("ID: ", Magic);
   CheckPositions();
   fk = iMA(_Symbol, PERIOD_W1, len, 0, MODE_SMA, PRICE_CLOSE);
}
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         CopyBuffer(MAHandler, 0, 0, 4, MA);
         CheckPositions();
         
         if(BullishCandle(1) && ((BullishCandle(2) && Price[2].close>MA[2]) || Confirmation==false) && Price[1].close>MA[1]){
            if(buyPosition==0) ExecuteBuy();
         }
            
         if(BullishCandle(1)==false && Price[1].close<MA[1]){
...
void PositionSize(){
   if(FixedRisk>0){
      if(MinLot<=0.01){
         Lot = int((initial_balance*FixedRisk)/(ATR[1]*SlATR)*100);
         Lot/=100;
      }
      else Lot = int((initial_balance*FixedRisk)/(ATR[1]*SlATR));
   ...
   if(Lot>MinLot*10) Lot=MinLot*10;
}
...
bool BullishCandle(int num){
   if(Price[num].close>Price[num].open){
      return true;
   }
   else return false;
}