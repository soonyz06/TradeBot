...
input int TpPoints1 = -1; //Tp 
int TpPoints2 = TpPoints1; 
input int SlPoints = 1500; //Sl
input int TimeExit1 = 10; //Time Exit
...
void OnInit(){
   ...
   MAHandler = iMA(_Symbol, Timeframe, 150, 0, MODE_SMA, PRICE_CLOSE);
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){
         ...
         //Entry
         if(Price[1].close>Upper[1] && Price[2].close<Upper[2] && Price[1].close>MA[1]){
            if(buyPosition+sellPosition==0)
               ExecuteSell();
         }
         if(Price[1].close<Lower[1] && Price[2].close>Lower[2] && Price[1].close<MA[1]){
            if(buyPosition+sellPosition==0)
               ExecuteBuy();
         }
...
void CheckPositions(){  
   ...
   if(report){
      if(WasInPosition && buyDuration<0 && sellDuration<0) SendNotification(DoubleToString(Magic)+" Position Exited");
      else if(WasInPosition==false && buyDuration>=0) SendNotification(DoubleToString(Magic)+" Long Position Entered");
      else if(WasInPosition==false && sellDuration>=0) SendNotification(DoubleToString(Magic)+" Short Position Entered");
   }
}

void TimeExit(){
   if(TimeExit1<0 && buyDuration>=0) return; 
   if(TimeExit2<0 && sellDuration>=0) return;
...
void ModifyPosition(ENUM_POSITION_TYPE type){
   if(TrailPoints<0) return;
      ...
      if (PositionSelectByTicket(_ticket))
      {
         if (PositionGetInteger(POSITION_MAGIC)==Magic && PositionGetString(POSITION_SYMBOL) == _Symbol)
         {
            if(type==POSITION_TYPE_BUY){
               _sl = PositionGetDouble(POSITION_PRICE_OPEN)-TrailPoints*_Point;
            }
            if(type==POSITION_TYPE_SELL){
               _sl = PositionGetDouble(POSITION_PRICE_OPEN)+TrailPoints*_Point;
            }
            if(type == PositionGetInteger(POSITION_TYPE) && _sl!=PositionGetDouble(POSITION_SL)){
               trade.PositionModify(_ticket, _sl, PositionGetDouble(POSITION_TP));
            }
         }
      }
   }
