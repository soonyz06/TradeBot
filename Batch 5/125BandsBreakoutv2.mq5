...
input int SlMode = 0; //SL Mode
int TpATR; //TP
int SlATR; //SL
int TimeExit; //Time Exit
int bb_len; //BB Length
int std; //Standard Deviation
input int Confirmation = 100;
int trend_index; //Trend Index
int atr_len;
int Spread;
...
void OnInit(){
   ...
   if(SlMode==0) SlTimeframe = PERIOD_H1; else SlTimeframe = PERIOD_D1; 
   ATRHandler = iATR(_Symbol, SlTimeframe, atr_len);
...
void OnTick(){
   ...
   if(bar1!=newbar){ //precise
      if(TimeCurrent()>=Start && TimeCurrent()<Stop){
         ...
         CopyClose(_Symbol, Timeframe, trend_index, 1, Trend);
         CheckPositions();
         Range(Confirmation);
         
         if(Price[1].close>Highest && Price[1].close>Upper[1] && Price[2].close<Upper[2] && (Price[1].close>Trend[0] || trend_index<=0)){
            if(buyPosition==0)
            ExecuteBuy();
         }
         if(Price[1].close<Lowest && Price[1].close<Lower[1] && Price[2].close>Lower[2] && (Price[1].close<Trend[0] || trend_index<=0)){
            if(sellPosition==0)
            ExecuteSell();
         }
         bar1=newbar;
