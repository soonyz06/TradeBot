...
input int TpPoints1 = 4500; //Tp
int TpPoints2 = TpPoints1; //Short Tp
input int SlPoints = 1600;
input int TimeExit1= 31;
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start){         
         ...
         if(Price[1].close<Price[2].low && Price[2].close<Price[2].open && Price[1].close<Lowest){ 
            if(buyPosition==0 && sellPosition==0){ 
               ExecuteBuyStop(Price[1].high);
               SendNotification("Buy Stop"+string(_Symbol));
            }
         }
         
         if(Price[1].close>Price[2].high && Price[2].close>Price[2].open && Price[1].close>Highest){ 
            if(sellPosition==0 && buyPosition==0){
               ExecuteSellStop(Price[1].low);
               SendNotification("Sell Stop"+string(_Symbol));
            }
         }  
         bar1=newbar;
      }
...
void Range(int bars){
   ...
   CopyHigh(_Symbol, Timeframe, 2, bars, High); //copy close?
