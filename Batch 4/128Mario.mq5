...
void OnInit(){
   ...
   ObjectSetInteger(0, "Trend", OBJPROP_COLOR, clrWhite);
...
void OnTick(){
   ...
   if(bar1!=newbar){
      if(TimeCurrent()>=Start && TimeCurrent()<Stop){
         ...
         ObjectMove(0, "Trend", 0, _Period, trend[0]);
         CheckPositions();
         
         if(Price[1].close<Price[2].low && (Price[1].close>trend[0] || trend_index<=0))
            if(buyPosition+sellPosition==0)
               ExecuteBuyStop(Price[1].high);
         if(Price[1].close>Price[2].high && (Price[1].close<trend[0] || trend_index<=0)) 
            if(buyPosition+sellPosition==0)
